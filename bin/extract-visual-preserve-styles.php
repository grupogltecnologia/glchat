...
<?php
/**
 * Extrair HTML/CSS mantendo Tailwind e estilos, removendo apenas JavaScript de lógica
 */
...
echo "🎨 Extraindo HTML/CSS preservando estilos...\n\n";
...
foreach ($files as $htmlFile => $phpFile) {
    ...
    // 1. REMOVER APENAS <script type="module"> (lógica do Supabase)
    $content = preg_replace('/<script\s+type=["\']module["\']\s*>.*?<\/script>/s', '', $content);
    
    // 2. Manter <script> de configuração do Tailwind e estilos
    // (não remover scripts que não sejam type="module")
    
    // 3. Corrigir URLs do Supabase Storage
    ...
    // 5. Adicionar script de lógica ANTES do </body>
    ...
    // 6. Adicionar comentário PHP
    $phpComment = "<?php\n/**\n * Visual original do n8n - JavaScript reescrito para PHP/MySQL\n */\n?>\n";
    $content = $phpComment . $content;
    
    file_put_contents($destPath, $content);
    echo "   ✅ HTML/CSS preservado, apenas lógica JavaScript removida\n";
}

echo "\n✅ Processamento concluído!\n";
echo "📝 Tailwind CSS e estilos mantidos, apenas lógica Supabase removida\n";
